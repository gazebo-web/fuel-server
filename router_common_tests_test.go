...
func TestInvalidServerKey(t *testing.T) {
	...
	for _, uri := range uris {
		t.Run(uri, func(t *testing.T) {
			...
			code, bslice, ok := igntest.SendMultipartPOST(t.Name(), t, uri, &jwt, extraParams, files)
...
func TestCreateNamedResource(t *testing.T) {
	...
	var worldFiles = []igntest.FileDesc{
	...
	createResourceTestsData := []createResourceTest{
		...
		{uriTest{"duplicate model", uri, jwtDef,
			ign.NewErrorMessage(ign.ErrorFormDuplicateModelName), false},
			map[string]string{"name": "mo1", "owner": username, "license": "1"},
			modelFiles},
		...
		// WORLDS
		{uriTest{"no jwt", wURI, nil, ign.NewErrorMessage(ign.ErrorUnauthorized),
		...
		{uriTest{"dup world", wURI, jwtDef,
			ign.NewErrorMessage(ign.ErrorFormDuplicateWorldName), false},
			map[string]string{"name": "wo1", "owner": username, "license": "1"},
			worldFiles},
	...
	for _, test := range createResourceTestsData {
		t.Run(test.testDesc, func(t *testing.T) {
			...
			code, bslice, _ := igntest.SendMultipartPOST(t.Name(), t, test.URL, jwt,
				test.params, test.files)
			assert.Equal(t, expStatus, code)
			if expStatus != http.StatusOK && !test.ignoreErrorBody {
				igntest.AssertBackendErrorCode(t.Name()+" POST "+test.URL, bslice,
					expEm.ErrCode, t)
			}
		})
	}
...
func TestDeleteWorldsAndModels(t *testing.T) {
	...
	deleteTestsData := []deleteNamedResourceTest{
		...
		// WORLDS
		{uriTest{"cannot delete world with another jwt", w1URI, newJWT(jwt2), ign.NewErrorMessage(ign.ErrorUnauthorized), false}, "", "", &worlds.World{}},
		{uriTest{"cannot delete world with no jwt", w1URI, nil, ign.NewErrorMessage(ign.ErrorUnauthorized), true}, "", "", &worlds.World{}},
		...
		// MODELS
		{uriTest{"cannot delete model with another jwt", m1URI, newJWT(jwt2), ign.NewErrorMessage(ign.ErrorUnauthorized), false}, "", "", &models.Model{}},
		{uriTest{"cannot delete model no jwt", m1URI, nil, ign.NewErrorMessage(ign.ErrorUnauthorized), true}, "", "", &models.Model{}},
...
func TestGetFileTree(t *testing.T) {
	...
	fileTreeTestData := []getFileTreeTest{
		...
		{uriTest{"invalid model", worldURL(testUser, "invalidmodel", "") + "/tip/files.json", nil, ign.NewErrorMessage(ign.ErrorNameNotFound), true}, 0, 1, nil},
		{uriTest{"model version", modelURL(testUser, "model1", "") + "/1/files", nil, nil, false}, 3, 1, expModelPaths},
		{uriTest{"model invalid version format #1", modelURL(testUser, "model1", "") + "/-1.23/files", nil, ign.NewErrorMessage(ign.ErrorFormInvalidValue), false}, 0, 1, nil},
		{uriTest{"model invalid version format #2", modelURL(testUser, "model1", "") + "/-a0/files", nil, ign.NewErrorMessage(ign.ErrorFormInvalidValue), false}, 0, 1, nil},
		{uriTest{"model version 0", modelURL(testUser, "model2", "") + "/0/files", nil, ign.NewErrorMessage(ign.ErrorFormInvalidValue), false}, 0, 1, nil},
		{uriTest{"unexistent model version", modelURL(testUser, "model2", "") + "/5/files", nil, ign.NewErrorMessage(ign.ErrorVersionNotFound), false}, 0, 1, nil},
		...
		{uriTest{"get private model with non member", modelURL(orgname, "orgModel", "") + "/tip/files", newJWT(jwt2), ign.NewErrorMessage(ign.ErrorUnauthorized), false}, 0, 1, nil},
		// // WORLDS
		...
		{uriTest{"invalid world", worldURL(testUser, "invalidworld", "") + "/tip/files.json", nil, ign.NewErrorMessage(ign.ErrorNameNotFound), true}, 0, 1, nil},
		{uriTest{"world version", worldURL(testUser, "world1", "") + "/1/files", nil, nil, false}, 3, 1, expWorldPaths},
		{uriTest{"world invalid version format #1", worldURL(testUser, "world1", "") + "/-1.23/files", nil, ign.NewErrorMessage(ign.ErrorFormInvalidValue), false}, 0, 1, nil},
		{uriTest{"world invalid version format #2", worldURL(testUser, "world1", "") + "/-a0/files", nil, ign.NewErrorMessage(ign.ErrorFormInvalidValue), false}, 0, 1, nil},
		{uriTest{"world version 0", worldURL(testUser, "world2", "") + "/0/files", nil, ign.NewErrorMessage(ign.ErrorFormInvalidValue), false}, 0, 1, nil},
		{uriTest{"unexistent world version", worldURL(testUser, "world2", "") + "/5/files", nil, ign.NewErrorMessage(ign.ErrorVersionNotFound), false}, 0, 1, nil},
		...
		{uriTest{"get private world with non member", worldURL(orgname, "orgWorld", "") + "/tip/files", newJWT(jwt2), ign.NewErrorMessage(ign.ErrorUnauthorized), false}, 0, 1, nil},
	}
	...
	brokenModel := getFileTreeTest{uriTest{"broken model1 filetree", modelURL(testUser, "model1", "") + "/tip/files", nil, ign.NewErrorMessage(ign.ErrorUnexpected), false}, 0, 1, nil}
	...
	brokenWorld := getFileTreeTest{uriTest{"broken world1 filetree", worldURL(testUser, "world1", "") + "/tip/files", nil, ign.NewErrorMessage(ign.ErrorUnexpected), false}, 0, 1, nil}
...
func TestFileTreeProto(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.uri, func(t *testing.T) {
			...
			igntest.AssertRoute("OPTIONS", test.uri, http.StatusOK, t)
			reqArgs := igntest.RequestArgs{Method: "GET", Route: test.uri, Body: nil, SignedToken: &myJWT}
			resp := igntest.AssertRouteMultipleArgsStruct(reqArgs, http.StatusOK, "application/arraybuffer", t)
...
func TestGetIndividualFile(t *testing.T) {
	...
	individualFileTestsDaa := []individualFileTest{
		...
		{uriTest{"model invalid file", m1URI + "/tip/files/invalid.sdf", nil, ign.NewErrorMessage(ign.ErrorFileNotFound), false}, "", 1},
		{uriTest{"invalid model name", modelURL(testUser, "invalid", "") + "/tip/files/model.sdf", nil, ign.NewErrorMessage(ign.ErrorNameNotFound), false}, "", 1},
		{uriTest{"model file from subfolder", m2URI + "/tip/files/subfolder/test.txt", nil, nil, false}, "text/plain; charset=utf-8", 1},
		{uriTest{"model subfolder only", m2URI + "/tip/files/subfolder/", nil, ign.NewErrorMessage(ign.ErrorFileNotFound), false}, "", 1},
		{uriTest{"model subfolder no slash", m2URI + "/tip/files/subfolder", nil, ign.NewErrorMessage(ign.ErrorFileNotFound), false}, "", 1},
		...
		{uriTest{"model invalid version", m1URI + "/0/files/model.config", nil, ign.NewErrorMessage(ign.ErrorFormInvalidValue), false}, "", 1},
		{uriTest{"model version not found", m1URI + "/2/files/model.config", nil, ign.NewErrorMessage(ign.ErrorVersionNotFound), false}, "", 1},
		...
		{uriTest{"get private model with non member", pmURI + "/tip/files/model.config", newJWT(jwt2), ign.NewErrorMessage(ign.ErrorUnauthorized), false}, "", 1},
		{uriTest{"get private user model with another jwt", modelURL(testUser, "user_private", "") + "/tip/files/model.config", newJWT(jwt2), ign.NewErrorMessage(ign.ErrorUnauthorized), false}, "", 1},
		// WORLDS
		{uriTest{"world sdf", w2URI + "/tip/files/world.world", nil, nil, false}, "text/xml; charset=utf-8", 1},
		{uriTest{"world invalid file", w1URI + "/tip/files/invalid.sdf", nil, ign.NewErrorMessage(ign.ErrorFileNotFound), false}, "", 1},
		{uriTest{"invalid world name", worldURL(testUser, "invalid", "") + "/tip/files/model.sdf", nil, ign.NewErrorMessage(ign.ErrorNameNotFound), false}, "", 1},
		{uriTest{"world file from subfolder", w2URI + "/tip/files/subfolder/test.txt", nil, nil, false}, "text/plain; charset=utf-8", 1},
		{uriTest{"world subfolder only", w2URI + "/tip/files/subfolder/", nil, ign.NewErrorMessage(ign.ErrorFileNotFound), false}, "", 1},
		{uriTest{"world subfolder no slash", w2URI + "/tip/files/subfolder", nil, ign.NewErrorMessage(ign.ErrorFileNotFound), false}, "", 1},
		...
		{uriTest{"world invalid version", w1URI + "/0/files/world.world", nil, ign.NewErrorMessage(ign.ErrorFormInvalidValue), false}, "", 1},
		{uriTest{"world version not found", w1URI + "/2/files/world.world", nil, ign.NewErrorMessage(ign.ErrorVersionNotFound), false}, "", 1},
		...
		{uriTest{"get private world with non member", pwURI + "/tip/files/world.world", newJWT(jwt2), ign.NewErrorMessage(ign.ErrorUnauthorized), false}, "", 1},
		{uriTest{"get private user world with another jwt", worldURL(testUser, "user_private", "") + "/tip/files/world.world", newJWT(jwt2), ign.NewErrorMessage(ign.ErrorUnauthorized), false}, "", 1},
	}
...
func TestOptionsIndividualFile(t *testing.T) {
	...
	igntest.AssertRoute("OPTIONS", wURI, http.StatusOK, t)
	igntest.AssertRoute("OPTIONS", wSubFolder, http.StatusOK, t)
}
